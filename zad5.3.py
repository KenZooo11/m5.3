...
def create_contacts(rodzaj_wizytowki, ilosc):
    ...
    for _ in range(ilosc):
        ...
        if rodzaj_wizytowki.lower() == "prywatnykontakt":
            ...
            contact = SluzbowyKontakt(first_name, last_name, email, phone_number, position, company, business_phone)
        else:
...
if __name__ == "__main__":
    ...
    for contact in contacts:
        contact.contact()
        print(f"\nDługość imienia i nazwiska dla {contact.first_name} {contact.last_name}: {contact.label_length}\n")
        print(f"Imię: {contact.first_name}")
        print(f"Nazwisko: {contact.last_name}")
        if isinstance(contact, SluzbowyKontakt):
            print(f"Firma: {contact.company}")
            print(f"Stanowisko: {contact.position}")
